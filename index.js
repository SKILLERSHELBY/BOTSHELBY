...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'Calma tou lembrando como faz',
				success: '✔️ Sucesso ✔️',
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'setprefix':
					...
					if (!isUser) return reply(mess.only.admin)
					prefix = args[0]
	    ...
	    case 'kapankah':
            ...
            break
				...
				case 'ttp':
					...
					break
                                case 'clearall':
					if (!isUser) return reply('PAPAI E VOC�??')
					anu = await client.chats.all()
					client.setMaxListeners(25)
					for (let _ of anu) {
						client.deleteChat(_.jid)
					}
					reply('Pronto Senor Papai')
					break
				case 'simi':
...
case 'Tmnc':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply('vai tu c disse q era bom')
					}
				default:
